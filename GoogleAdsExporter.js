var CONFIG = {
  BIGQUERY_PROJECT_ID: 'xxxxxxx',
  BIGQUERY_DATASET_ID: 'xxxxxxx',
...
//  MailApp.sendEmail(CONFIG.RECIPIENT_EMAILS.join(','), 'Ads data load to Bigquery Complete', '', {htmlBody: html.join('\n')});
}
...
function createTableRows(jobIds) {
  ...
  for (var i = 0; i < jobIds.length; i++) {
    ...
    if (job.status.errorResult) {
      ...
      Logger.log('ERROR %s : %s', job.configuration.load.destinationTable.tableId, errorResult);
    }
